...
export class UserAccountsController {
    ...
    constructor(private usersService: UserAccountService, private mapper: UserAccountsResourceMapper) { }
    ...
    public async getUsers(): Promise<UserAccountResource[]> {
        return this.usersService.getUsers().then(users => this.mapper.toResources(users));
    }
    ...
    public getCurrentUser(@Req() req: Request): Promise<UserAccountResource> {
        ...
        return this.usersService.findById((req.user as any).userId).then(user => this.mapper.toResource(user));
    }
