...
export class LoginController {
    constructor(private authService: AuthService, private mapper: LoginResourceMapper) { }

    @UseGuards(AuthGuard('local'))
    @Post()
    public async login(@Req() req: Request, @Res() res: Response): Promise<void> {
        const loginResource = this.mapper.toResource(await this.authService.login(req.user));
        res
