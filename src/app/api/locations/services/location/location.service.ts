...
export class LocationService {
    ...
    public async createOrUpdateLocation(userId: string, latitude: number, longitude: number): Promise<void> {
        const location = this.locations.find(l => l.getUserId() === userId);
        if (location) {
            location.setLatitude(latitude);
            location.setLongitude(longitude);
        } else {
            this.locations.push(new LocationEntity(userId, new Date(), latitude, longitude));
        }
    }
